...
func RegisterEmbeddingSearchTool(server *mcp.Server, svc primary.EmbeddingSearch, log *slog.Logger) {
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in embeddingSearchInput) (*mcp.CallToolResult, domain.EmbeddingSearchResponse, error) {
		if log != nil {
			log.DebugContext(ctx, "mosaic_hexxla_search_embedding invoked")
		}
		out, err := svc.Search(ctx, domain.EmbeddingSearchQuery{
			Text:       in.Query,
