...
func RegisterFacetEdgeBrowseTools(server *mcp.Server, svc primary.FacetEdgeBrowse, log *slog.Logger) {
	registerGetFacet(server, svc, log)
	registerListFacets(server, svc, log)
	registerGetEdge(server, svc, log)
	registerListEdgesFrom(server, svc, log)
}

func registerGetFacet(server *mcp.Server, svc primary.FacetEdgeBrowse, log *slog.Logger) {
	type input struct {
		Q       int  `json:"q"`
		R       int  `json:"r"`
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in input) (*mcp.CallToolResult, domain.GetFacetResponse, error) {
		if log != nil {
			log.DebugContext(ctx, "mosaic_hexxla_get_facet")
		}
		out, err := svc.GetFacet(ctx, domain.AxialCoord{Q: in.Q, R: in.R}, in.FacetID)
		return nil, out, err
	})
}

func registerListFacets(server *mcp.Server, svc primary.FacetEdgeBrowse, log *slog.Logger) {
	type input struct {
		Q int `json:"q"`
		R int `json:"r"`
	}
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in input) (*mcp.CallToolResult, domain.ListFacetsForCellResponse, error) {
		if log != nil {
			log.DebugContext(ctx, "mosaic_hexxla_list_facets")
		}
		out, err := svc.ListFacetsForCell(ctx, domain.AxialCoord{Q: in.Q, R: in.R})
		return nil, out, err
	})
}

func registerGetEdge(server *mcp.Server, svc primary.FacetEdgeBrowse, log *slog.Logger) {
	type input struct {
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in input) (*mcp.CallToolResult, domain.GetEdgeResponse, error) {
		if log != nil {
			log.DebugContext(ctx, "mosaic_hexxla_get_edge")
		}
		out, err := svc.GetEdge(ctx,
...
func registerListEdgesFrom(server *mcp.Server, svc primary.FacetEdgeBrowse, log *slog.Logger) {
	type input struct {
		FromQ    int `json:"from_q"`
		FromR    int `json:"from_r"`
	...
	}, func(ctx context.Context, _ *mcp.CallToolRequest, in input) (*mcp.CallToolResult, domain.ListEdgesFromResponse, error) {
		if log != nil {
			log.DebugContext(ctx, "mosaic_hexxla_list_edges_from")
		}
		out, err := svc.ListEdgesFrom(ctx, domain.AxialCoord{Q: in.FromQ, R: in.FromR}, in.MaxEdges)
		return nil, out, err
	})
}
